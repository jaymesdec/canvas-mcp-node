...
## v0.4.2 — assessment template requires FAIR/FINAL flags

- **`template: 'assessment'` now requires `final_asmt` and `fair_asmt`** on `create_assignment`, and on `update_assignment` when it re-applies the assessment template. `final_asmt` = the assessment counts toward the student's final grade (FINAL title tag); `fair_asmt` = it's published to Franklin's continuous reporting tool where parents see the assessment, grade, graded rubric, and comments (FAIR title tag). The server fails closed before any Canvas call when either flag is missing — **the model will ask the teacher both questions rather than assume**. Skills that create assessments should collect both answers up front and pass the flags explicitly.
- **Title-convention warnings + `suggested_title_flags`.** Assessment writes now check the assignment name against the `{fair_flag}{final_flag}ASMT` title convention and return `warnings[]` on mismatch (missing `ASMT` suffix, missing or stray `FAIR`/`FINAL` tokens) — warnings only, the name is the teacher's call. The response also carries `suggested_title_flags` (`""` | `"FAIR "` | `"FINAL "` | `"FAIR FINAL "`, FAIR before FINAL) so the model can compose titles correctly.

---

## v0.4.3 — week numbers now come from the official calendar table
...
## v0.4.5 — precise week diagnostics + assessment conversation conduct

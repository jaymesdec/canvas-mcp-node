...
* [ ] **C5.** "Create an assessment called 'MCP Test ASMT' using the assessment template with these requirements: \[a few bullets]." → Expect Claude to **ask before creating**: "Which assignment group does this belong to?" (listing your course's real groups with their weights), "Is this a FINAL assessment (counts toward the final grade)?", "Is it a FAIR assessment (published to the continuous reporting tool for parents)?" — pick a group, answer one yes and one no. Check Canvas: the assignment landed **in that group**, the description shows Franklin chrome + the assessment sections, and Claude proposed a full composed title like `Project: MCP Test [Week N] 15% FINAL ASMT` with the percent taken from the group's Canvas weight and the week from the due date. Then: "Create a quick assignment called 'MCP Plain Test' with a one-line description." → no group/FAIR/FINAL questions; description shows the default chrome wrap.

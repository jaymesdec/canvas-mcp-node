...
| `create_assignment(course_identifier, name, description?, due_at?, unlock_at?, lock_at?, points_possible?, submission_types?, template?, final_asmt?, fair_asmt?, assignment_group?, asmt_percent?, slots?, include_sections?, omit_sections?)` | Create an assignment. **`published: false` forced.** Description is wrapped in the school's `default` page template automatically; `template: 'assessment'` (etc.) picks a named template, `'none'` skips wrapping — same slot/section machinery as `create_page`. No description/slots → no description sent at all. **`template: 'assessment'` requires `final_asmt` (counts toward the final grade), `fair_asmt` (published to the parent-facing continuous reporting tool), and `assignment_group`** (id or exact name — the group is the title's `{type}` and its Canvas `group_weight` the `{percent}`) — the model asks the teacher, never assumes. The response carries a composed `suggested_title` (week from the due date via the calendar table, percent from Canvas), `resolved_percent`, `suggested_title_flags`, and title-tag warnings. `asmt_percent` is the fallback when the group has no usable weight; `assignment_group` also works on plain assignments as a group placement. |
| `update_assignment(course_identifier, assignment_id, name?, description?, due_at?, unlock_at?, lock_at?, points_possible?, submission_types?, template?, final_asmt?, fair_asmt?, assignment_group?, asmt_percent?, slots?, include_sections?, omit_sections?)` | Update assignment fields. Simple mode sends `description` verbatim and supports `assignment_group` as a plain regroup; passing `template`/`slots`/`include_sections`/`omit_sections` rebuilds the description from the school template (pass ALL slots you want in the result). Re-applying `template: 'assessment'` requires `final_asmt`/`fair_asmt`/`assignment_group` (same ask-the-teacher rule, `suggested_title` composition, and title-tag warnings as `create_assignment`). Never touches published state; warns when Canvas silently ignores a `submission_types` change (happens once students have submitted). |
| `list_assignment_groups(course_identifier)` | List the course's assignment groups (`id`, `name`, `position`, `group_weight`) plus the course's `apply_assignment_group_weights` flag. For assessments the group name is the title's `{type}` and its weight the `{percent}` — ask the teacher which group, then take the percent from this tool, not from memory. Notes when weights aren't applied to grades. |
